  ...
  return [...flags].sort((a, b) => {
    ...
    switch (config.column) {
      ...
      case 'value':
        ...
        break;
      case 'preset':
        cmp = Number(a.preset) - Number(b.preset);
        break;
      case 'tags':
  ...
  return (
    <div className="flex-1 flex items-center justify-center">
      <div className="text-center space-y-3 max-w-xs">
        ...
        <h3 className="font-medium" style={{ color: 'var(--ff-text-secondary)' }}>
          No flags yet
        </h3>
        <p className="text-xs" style={{ color: 'var(--ff-text-muted)' }}>
          Add a flag, import a file, or paste JSON.
        </p>
    ...
    >
      {COLUMNS.map(col => (
        <div
          ...
          onClick={() => {
            if (col.key === 'tags' || col.key === 'preset') return;
            setSortConfig(col.key as SortConfig['column']);
          }}
        >
          {col.label}
          {sortConfig?.column === col.key &&
            (sortConfig.direction === 'asc' ? (
              <ChevronUpIcon className="w-2.5 h-2.5" style={{ color: 'var(--ff-accent)' }} />
            ) : (
              <ChevronDownIcon className="w-2.5 h-2.5" style={{ color: 'var(--ff-accent)' }} />
            ))}
        </div>
      ))}
    </div>
...
export function FlagTable() {
  ...
  const handleContextMenu = useCallback(
    (e: React.MouseEvent, flagId: string) => {
      e.preventDefault();
              ...
              return (
                <div
                  ...
                  onDoubleClick={e => {
                    ...
                    const col =
                      e.clientX - rect.left < 280
                        ? 'tags'
                        : e.clientX - rect.left < 320
                          ? 'preset'
                          : e.clientX - rect.left < rect.width / 2 + 160
                            ? 'name'
                            : 'value';
                    useUIStore.getState().startEditing({ flagId: flag.id, column: col });
                  }}
                >
                  {/* Tags column */}
                  <div
                  ...
                  <div
                    ...
                    onClick={e => {
                      e.stopPropagation();
                      togglePreset(flag.id);
                    }}
                  >
                    <div
                      className={`w-4 h-4 rounded flex items-center justify-center transition-colors ${flag.preset ? '' : ''}`}
                      style={{
                        background: flag.preset ? 'var(--ff-accent)' : 'var(--ff-bg-tertiary)',
                        border: flag.preset ? 'none' : '1px solid var(--ff-border)',
                      }}
                    >
                      {flag.preset && (
                  ...
                  {/* Name */}
                  <div
                    className="flex-1 px-2 text-xs truncate"
                  ...
                  {/* Value */}
                  <div
                    className="flex-1 px-2 text-[11px] truncate font-mono"
                    style={{
                      color:
                        flag.value === 'True'
                          ? '#34d399'
