...
export function StatusBar({ onWallpaperChange }: StatusBarProps) {
  ...
  return (
    <div
      className="flex items-center justify-between px-3 py-1 shrink-0 border-t text-[11px]"
      style={{
    ...
    >
      <div className="flex items-center gap-2">
        ...
        {presetCount > 0 && (
          <span style={{ color: 'var(--ff-accent)' }}>{presetCount} presets</span>
        )}
      </div>
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          onClick={togglePresetFilter}
          className="flex items-center gap-1 px-2 py-0.5 rounded text-[11px] transition-colors"
          style={{
            background: presetFilter ? 'var(--ff-accent)' : 'var(--ff-bg-tertiary)',
            color: presetFilter ? '#fff' : 'var(--ff-text-secondary)',
          }}
        >
